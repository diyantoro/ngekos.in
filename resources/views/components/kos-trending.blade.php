...
@if ($trending->isNotEmpty())
    <div>
        <div class="flex items-end justify-between gap-4">
            <div>
                <h2 class="text-lg font-extrabold text-gray-900">Kos Trending</h2>
                <p class="text-xs text-gray-500 mt-0.5">Kos paling laris &amp; banyak dicari</p>
            </div>
            <a href="{{ route('kos.index') }}" wire:navigate
                class="shrink-0 inline-flex items-center gap-0.5 text-sm font-semibold text-teal-600 hover:text-teal-700">
                Lihat Semua
        ...
        <div class="mt-4 flex gap-4 overflow-x-auto scrollbar-hide pb-2 -mb-2 snap-x snap-mandatory">
            @foreach ($trending as $p)
                <a href="{{ route('kos.detail', $p) }}" wire:navigate
                    ...
                    <div class="relative h-36 bg-gradient-to-br from-orange-50 to-amber-100 overflow-hidden">
                        ...
                        @else
                            <div class="h-full w-full flex items-center justify-center">
                                <svg class="h-10 w-10 text-teal-600" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" /></svg>
                            </div>
                        @endif
                    ...
                    <div class="p-3">
                        <p class="text-sm font-bold text-gray-900 truncate group-hover:text-teal-700 transition">{{ $p->nama }}</p>
                        <p class="mt-1 text-xs text-gray-500 truncate flex items-center gap-1">
                        ...
                        <div class="mt-3 flex items-center justify-between rounded-lg bg-teal-50 px-2.5 py-1.5">
                            <span class="text-[11px] font-bold text-teal-700">{{ $p->kamar_terisi }}/{{ $p->total_kamar }} terisi</span>
                            <svg class="h-3.5 w-3.5 text-teal-600" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18L9 11.25l4.306 4.307a11.95 11.95 0 015.814-5.519l2.74-1.22m0 0l-5.94-2.28m5.94 2.28l-2.28 5.941" /></svg>
                        </div>
                    </div>
                </a>
            @endforeach
