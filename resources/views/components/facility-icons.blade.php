...
@php
    $daftarFasilitas = [
        ...
        'Dapur Bersama' => '<path stroke-linecap="round" stroke-linejoin="round" d="M12 8.25v-1.5m0 1.5c-1.355 0-2.697.056-4.024.166C6.845 8.51 6 9.473 6 10.608v2.513m6-4.87c1.355 0 2.697.055 4.024.165C17.155 8.51 18 9.473 18 10.608v2.513m-3-4.87v-1.5m-6 1.5v-1.5m12 9.75l-1.5.75a3.354 3.354 0 01-3 0 3.354 3.354 0 00-3 0 3.354 3.354 0 01-3 0 3.354 3.354 0 00-3 0 3.354 3.354 0 01-3 0L3 16.5m15-3.38a48.474 48.474 0 00-6-.37c-2.032 0-4.034.126-6 .37m12 0c.39.049.777.102 1.163.16 1.07.16 1.837 1.094 1.837 2.175v5.17c0 .62-.504 1.124-1.125 1.124H4.125A1.125 1.125 0 013 20.625v-5.17c0-1.08.768-2.014 1.837-2.174A47.78 47.78 0 016 13.12M12.265 3.11a.375.375 0 11-.53 0L12 2.845l.265.265zm-3 0a.375.375 0 11-.53 0L9 2.845l.265.265zm6 0a.375.375 0 11-.53 0L15 2.845l.265.265z" />',
...
@if ($editable)
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
        @foreach ($daftarFasilitas as $nama => $ikon)
            @php $isChecked = in_array($nama, $selected); @endphp
            <label class="relative cursor-pointer">
                <input type="checkbox" value="{{ $nama }}" wire:model="fasilitasTerpilih"
                    class="peer sr-only">
                <span :class="$isChecked ? 'border-teal-500 bg-teal-50 shadow-sm ring-teal-200' : 'border-gray-200 bg-white hover:border-gray-300 hover:bg-gray-50'"
                    class="flex items-center gap-2.5 rounded-xl border-2 p-3 transition-all duration-200">
                    <span :class="$isChecked ? 'bg-teal-600 text-white' : 'bg-gray-100 text-gray-400'"
                        class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg transition-colors duration-200">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">{!! $ikon !!}</svg>
                    </span>
                    <span class="text-xs font-semibold text-gray-700">{{ $nama }}</span>
                    <span x-show="$isChecked" x-cloak
        ...
        @foreach (array_filter(array_map('trim', explode(',', $fasilitas))) as $f)
            @if (isset($daftarFasilitas[$f]))
                <span class="inline-flex items-center gap-1.5 rounded-lg bg-teal-50 px-3 py-2 text-xs font-medium text-teal-700 ring-1 ring-inset ring-teal-200">
                    <svg class="h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">{!! $daftarFasilitas[$f] !!}</svg>
                    {{ $f }}
                </span>
            @else
