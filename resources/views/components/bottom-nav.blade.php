...
<div class="fixed bottom-0 left-0 right-0 z-50 sm:hidden safe-bottom">
    ...
    <nav class="relative flex items-center justify-around h-16 px-2">
        {{-- Active indicator pill (liquid glass) --}}
        <div x-data="{
            activeIndex: @js(array_search(true, [
                $isHome,
                request()->routeIs('kos.*'),
            ...
            <div x-ref="nav" class="relative flex items-center justify-around w-full gap-1" role="navigation">
                ...
                {{-- Home --}}
                <a href="{{ $berandaUrl }}" wire:navigate
                    data-nav-item
                    ...
                    class="relative flex flex-col items-center justify-center gap-0.5 w-16 py-1 transition-all duration-300 ease-out
                        {{ $isHome ? 'text-teal-600 dark:text-teal-400' : 'text-gray-400 dark:text-gray-500' }}
                        active:scale-95
                        focus:outline-none">
                    <svg class="h-6 w-6 transition-all duration-300 {{ $isHome ? 'scale-110 drop-shadow-[0_4px_8px_rgba(16,185,129,0.3)]' : '' }}"
                        fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
                    </svg>
                    <span class="text-[10px] font-semibold transition-colors duration-200">{{ __('Beranda') }}</span>
                </a>
                ...
                <a href="{{ route('kos.index') }}" wire:navigate
                    ...
                    x-on:mouseleave="event.target.style.transform = ''"
                    class="relative flex flex-col items-center justify-center gap-0.5 w-16 py-1 transition-all duration-300 ease-out
                        {{ request()->routeIs('kos.*') ? 'text-teal-600 dark:text-teal-400' : 'text-gray-400 dark:text-gray-500' }}
                        active:scale-95
                        focus:outline-none">
                    <svg class="h-6 w-6 transition-all duration-300 {{ request()->routeIs('kos.*') ? 'scale-110 drop-shadow-[0_4px_8px_rgba(16,185,129,0.3)]' : '' }}"
                        fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
                    </svg>
                    <span class="text-[10px] font-semibold transition-colors duration-200">{{ __('Cari Kos') }}</span>
                </a>
                ...
                @auth
                    ...
                    <a href="{{ route('chat.index') }}" wire:navigate
                        ...
                        x-on:mouseleave="event.target.style.transform = ''"
                        class="relative flex flex-col items-center justify-center gap-0.5 w-16 py-1 transition-all duration-300 ease-out
                            {{ request()->routeIs('chat.*') ? 'text-teal-600 dark:text-teal-400' : 'text-gray-400 dark:text-gray-500' }}
                            active:scale-95
                            focus:outline-none">
                        <svg class="h-6 w-6 transition-all duration-300 {{ request()->routeIs('chat.*') ? 'scale-110 drop-shadow-[0_4px_8px_rgba(16,185,129,0.3)]' : '' }}"
                            fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                        ...
                        @if ($unread > 0)
                            <span class="absolute top-0.5 right-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-gradient-to-r from-rose-500 to-pink-500 text-[9px] font-bold text-white shadow-sm shadow-rose-500/30 animate-bounce-gentle z-10">{{ $unread > 9 ? '9+' : $unread }}</span>
                        @endif
                        <span class="text-[10px] font-semibold transition-colors duration-200">{{ __('Pesan') }}</span>
                    </a>
                    ...
                    @if (auth()->user()->hasAnyRole(['super_admin', 'admin']))
                        ...
                        <a href="{{ route('bantuan.masuk') }}" wire:navigate
                            ...
                            x-on:mouseleave="event.target.style.transform = ''"
                            class="relative flex flex-col items-center justify-center gap-0.5 w-16 py-1 transition-all duration-300 ease-out
                                {{ request()->routeIs('bantuan.masuk') ? 'text-teal-600 dark:text-teal-400' : 'text-gray-400 dark:text-gray-500' }}
                                active:scale-95
                                focus:outline-none">
                            <svg class="h-6 w-6 transition-all duration-300 {{ request()->routeIs('bantuan.masuk') ? 'scale-110 drop-shadow-[0_4px_8px_rgba(16,185,129,0.3)]' : '' }}"
                                fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                            ...
                            @if ($bantuanBaru > 0)
                                <span class="absolute top-0.5 right-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-gradient-to-r from-rose-500 to-pink-500 text-[9px] font-bold text-white shadow-sm shadow-rose-500/30 animate-bounce-gentle z-10">{{ $bantuanBaru > 9 ? '9+' : $bantuanBaru }}</span>
                            @endif
                            <span class="text-[10px] font-semibold transition-colors duration-200">{{ __('Bantuan') }}</span>
                        </a>
                    @endif
                    ...
                    @if (auth()->user()->hasAnyRole(['pemilik', 'admin', 'super_admin']))
                        ...
                        <div x-data="{ buka: false }" class="relative" data-nav-item>
                            <button @click="buka = !buka" type="button"
                                ...
                                x-on:mouseleave="event.target.style.transform = ''"
                                class="relative flex flex-col items-center justify-center gap-0.5 w-16 py-1 transition-all duration-300 ease-out
                                    {{ request()->routeIs('pemilik.*') ? 'text-teal-600 dark:text-teal-400' : 'text-gray-400 dark:text-gray-500' }}
                                    active:scale-95
                                    focus:outline-none">
                                <svg class="h-6 w-6 transition-all duration-300 {{ request()->routeIs('pemilik.*') ? 'scale-110 drop-shadow-[0_4px_8px_rgba(16,185,129,0.3)]' : '' }}"
                                    fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 21v-7.5a.75.75 0 01.75-.75h3a.75.75 0 01.75.75V21m-4.5 0H2.36m11.14 0H18m0 0h3.64m-1.39 0V9.349m-16.5 11.65V9.35m0 0a3.001 3.001 0 003.75-.615A2.993 2.993 0 009.75 9.75c.896 0 1.7-.393 2.25-1.016a2.993 2.993 0 002.25 1.016c.896 0 1.7-.393 2.25-1.016a3.001 3.001 0 003.75.614m-16.5 0a3.004 3.004 0 01-.621-4.72L4.318 3.44A1.5 1.5 0 015.378 3h13.243a1.5 1.5 0 011.06.44l1.19 1.189a3 3 0 01-.621 4.72m-13.5 8.65h3.75a.75.75 0 00.75-.75V13.5a.75.75 0 00-.75-.75H6.75a.75.75 0 00-.75.75v3.75c0 .414.336.75.75.75z" />
                                </svg>
                                <span class="text-[10px] font-semibold transition-colors duration-200">{{ __('Kelola') }}</span>
                            </button>
                            <div x-show="buka" x-cloak @click.outside="buka = false"
                                ...
                                <a href="{{ route('pemilik.properti') }}" wire:navigate @click="buka = false"
                                    class="flex items-center gap-3 px-4 py-3 text-sm font-semibold {{ request()->routeIs('pemilik.properti*') ? 'text-teal-600 dark:text-teal-400 bg-teal-500/10' : 'text-gray-700 dark:text-gray-200' }} hover:bg-white/50 dark:hover:bg-white/10 transition-all duration-200 rounded-xl mx-2 my-1">
                                ...
                                <a href="{{ route('pemilik.pengeluaran') }}" wire:navigate @click="buka = false"
                                    class="flex items-center gap-3 px-4 py-3 text-sm font-semibold {{ request()->routeIs('pemilik.pengeluaran') ? 'text-teal-600 dark:text-teal-400 bg-teal-500/10' : 'text-gray-700 dark:text-gray-200' }} hover:bg-white/50 dark:hover:bg-white/10 transition-all duration-200 rounded-xl mx-2 my-1">
                    ...
                    @if (auth()->user()->hasRole('pemilik'))
                        ...
                        <a href="{{ route('pemilik.grafik') }}" wire:navigate
                            ...
                            x-on:mouseleave="event.target.style.transform = ''"
                            class="relative flex flex-col items-center justify-center gap-0.5 w-16 py-1 transition-all duration-300 ease-out
                                {{ request()->routeIs('pemilik.grafik') ? 'text-teal-600 dark:text-teal-400' : 'text-gray-400 dark:text-gray-500' }}
                                active:scale-95
                                focus:outline-none">
                            <svg class="h-6 w-6 transition-all duration-300 {{ request()->routeIs('pemilik.grafik') ? 'scale-110 drop-shadow-[0_4px_8px_rgba(16,185,129,0.3)]' : '' }}"
                                fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" />
                            </svg>
                            <span class="text-[10px] font-semibold transition-colors duration-200">{{ __('Grafik') }}</span>
                        </a>
                    @endif
                    ...
                    <a href="{{ route('pengaturan') }}" wire:navigate
                        ...
                        x-on:click="activeIndex = @php $idx = 4; if (auth()->check() && auth()->user()->hasAnyRole(['super_admin', 'admin'])) $idx++; if (auth()->check() && auth()->user()->hasAnyRole(['pemilik', 'admin', 'super_admin'])) $idx++; if (auth()->check() && auth()->user()->hasRole('pemilik')) $idx++; echo $idx; @endphp"
                        x-on:mousedown="event.target.style.transform = 'scale(0.92)'"
                        x-on:mouseup="event.target.style.transform = ''"
                        x-on:mouseleave="event.target.style.transform = ''"
                        class="relative flex flex-col items-center justify-center gap-0.5 w-16 py-1 transition-all duration-300 ease-out
                            {{ request()->routeIs('pengaturan') ? 'text-teal-600 dark:text-teal-400' : 'text-gray-400 dark:text-gray-500' }}
                            active:scale-95
                            focus:outline-none">
                        <svg class="h-6 w-6 transition-all duration-300 {{ request()->routeIs('pengaturan') ? 'scale-110 drop-shadow-[0_4px_8px_rgba(16,185,129,0.3)]' : '' }}"
                            fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                        ...
                        @if (auth()->user()->avatar_url)
                            <img src="{{ auth()->user()->avatar_url }}" alt="{{ auth()->user()->nama }}" class="h-6 w-6 rounded-full object-cover ring-2 ring-white/50 dark:ring-gray-700/50 transition-all duration-300 {{ request()->routeIs('pengaturan') ? 'ring-teal-500/50 scale-110' : '' }}">
                        @else
                            <span class="flex h-6 w-6 items-center justify-center rounded-full bg-gradient-to-br from-teal-500 to-emerald-500 text-[10px] font-bold text-white ring-2 ring-white/50 dark:ring-gray-700/50 transition-all duration-300 {{ request()->routeIs('pengaturan') ? 'ring-teal-500/50 scale-110' : '' }}">{{ auth()->user()->inisial }}</span>
                        @endif
                        <span class="text-[10px] font-semibold transition-colors duration-200">{{ __('Akun') }}</span>
                    </a>
                @else
                    {{-- Login --}}
                    <a href="{{ route('login') }}" wire:navigate
                        ...
                        <span class="text-[10px] font-semibold">{{ __('Masuk') }}</span>
                    </a>
                    {{-- Register --}}
                    <a href="{{ route('register') }}" wire:navigate
                        class="flex flex-col items-center justify-center gap-0.5 w-16 py-1 text-gray-400 dark:text-gray-500 active:scale-95 transition-all duration-300 ease-out focus:outline-none">
                        <svg class="h-6 w-6 transition-transform duration-300 active:scale-110" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zM4 19.235v-.11a6.375 6.375 0 0112.75 0v.109A12.318 12.318 0 0110.374 21c-2.331 0-4.512-.645-6.374-1.766z" /></svg>
                        <span class="text-[10px] font-semibold">{{ __('Daftar') }}</span>
                    </a>
                @endauth
            </div>
        </div>
