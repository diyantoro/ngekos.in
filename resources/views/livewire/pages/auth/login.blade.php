...
<div>
    ...
    <div class="text-center mb-6">
        <div class="mx-auto mb-3 h-16 w-16 overflow-hidden rounded-full {{ $peran === 'pemilik' ? 'bg-emerald-100 dark:bg-emerald-500/10' : 'bg-teal-100 dark:bg-teal-500/10' }}">
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Sebagai <span class="font-semibold {{ $peran === 'pemilik' ? 'text-emerald-600 dark:text-emerald-400' : 'text-teal-600 dark:text-teal-400' }}">{{ $peran === 'pemilik' ? 'Pemilik Kos' : 'Pencari Kos' }}</span>
        </p>
    </div>
    ...
    <form wire:submit="login" class="space-y-5">
        ...
        <div class="flex items-center justify-between">
            @if (Route::has('password.request'))
                <a class="underline text-sm text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500" href="{{ route('password.request') }}" wire:navigate>
                    Lupa password?
                </a>
            @endif
        ...
        <p class="text-center text-sm text-gray-500 dark:text-gray-400 pt-2 border-t border-gray-100 dark:border-gray-700">
            Belum punya akun?
            <a class="font-semibold text-teal-600 hover:text-teal-500 dark:text-teal-400 dark:hover:text-teal-300" href="{{ route('register') }}" wire:navigate>
                {{ __('Daftar sekarang') }}
            </a>
        </p>
    </form>
