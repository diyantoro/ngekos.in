    ...
    public function with(): array
    {
        ...
        return [
            ...
            'sewaans' => Penyewaan::query()
                ...
                ->get(),
            'funnelStages' => [
                ['label' => 'Kunjungan', 'sub' => 'seluruh calon penyewa dari properti Anda', 'nilai' => Penyewaan::whereHas('properti', fn ($q) => $q->where('pemilik_id', $id))->count()],
                ['label' => 'Aktif', 'sub' => 'penyewaan berstatus aktif', 'nilai' => Penyewaan::where('status', 'aktif')->whereHas('properti', fn ($q) => $q->where('pemilik_id', $id))->count()],
                ['label' => 'Tagihan', 'sub' => 'total tagihan yang terbit', 'nilai' => Tagihan::whereHas('penyewaan.properti', fn ($q) => $q->where('pemilik_id', $id))->count()],
                ['label' => 'Lunas', 'sub' => 'tagihan berstatus lunas', 'nilai' => Tagihan::where('status', 'lunas')->whereHas('penyewaan.properti', fn ($q) => $q->where('pemilik_id', $id))->count()],
            ],
            'pendapatanPerBulan' => Pembayaran::where('status', 'diverifikasi')
                ...
                ->keyBy('month')
                ->all(),
            'tagihanStatusPerBulan' => Tagihan::whereHas('penyewaan.properti', fn ($q) => $q->where('pemilik_id', $id))
