...
<div class="py-8">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Header -->
        <div class="bg-white rounded-2xl shadow-sm ring-1 ring-gray-100 p-4 flex items-center justify-between gap-3">
            <div class="flex items-center gap-3 min-w-0">
                <a href="{{ route('chat.index') }}" wire:navigate
                   class="shrink-0 h-9 w-9 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600 transition">
                ...
                <div class="min-w-0">
                    <p class="text-sm font-bold text-gray-900 truncate">{{ $lawan?->nama ?? 'Pengguna terhapus' }}</p>
                    <p class="text-xs text-gray-500 truncate">soal kos {{ $properti->nama }}</p>
                </div>
            </div>
            <a href="{{ route('kos.detail', $properti) }}" wire:navigate
               class="shrink-0 inline-flex items-center rounded-lg border border-teal-200 bg-teal-50 px-3 py-1.5 text-xs font-semibold text-teal-700 hover:bg-teal-100 transition">
                Lihat Kos
            </a>
        </div>

        <!-- Pesan -->
        <div id="ruang-pesan" class="mt-4 bg-white rounded-2xl shadow-sm ring-1 ring-gray-100 p-4 sm:p-5 h-[55vh] overflow-y-auto space-y-3" wire:poll.3s>
            @forelse ($pesans as $p)
                @php
                    $punya = $p->pengirim_id === auth()->id();
                @endphp
                <div class="flex {{ $punya ? 'justify-end' : 'justify-start' }}">
                    <div class="max-w-[75%] rounded-2xl px-4 py-2.5 {{ $punya ? 'bg-teal-600 text-white rounded-br-md' : 'bg-gray-100 text-gray-800 rounded-bl-md' }}">
                        <p class="text-sm whitespace-pre-wrap break-words">{{ $p->isi }}</p>
                        <p class="mt-1 text-[10px] {{ $punya ? 'text-teal-200' : 'text-gray-400' }}">{{ $p->created_at->translatedFormat('d M, H:i') }}</p>
                    </div>
                </div>
            @empty
                <div class="h-full flex flex-col items-center justify-center text-center py-10">
                    ...
                    <p class="mt-3 text-sm font-medium text-gray-500">Belum ada pesan.</p>
                    <p class="text-xs text-gray-400">Mulai percakapan dengan menyapa lewat kolom di bawah.</p>
                </div>
            @endforelse
        </div>
        ...
        <form wire:submit="kirim" class="mt-4">
            ...
            @enderror
            <div class="flex items-end gap-2 bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 p-2 focus-within:ring-teal-400 transition">
