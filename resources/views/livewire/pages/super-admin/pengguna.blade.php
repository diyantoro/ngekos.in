...
<div class="py-10">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Kelola Pengguna &amp; Peran</h1>
        ...
        <input type="text" wire:model.live.debounce.300ms="cari" placeholder="Cari nama atau email..."
            class="w-full sm:w-72 rounded-lg border-gray-300 text-sm focus:ring-teal-500 focus:border-teal-500">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50 transition" wire:key="user-{{ $user->id }}">
                                <td class="px-4 py-4">
                                    ...
                                    <div class="mt-1.5 flex flex-wrap gap-1.5">
                                        @foreach ($user->roles as $role)
                                            <span class="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ring-1 ring-inset {{ $role->name === 'super_admin' ? 'bg-emerald-50 text-emerald-700 ring-emerald-200' : 'bg-teal-50 text-teal-700 ring-teal-200' }}">
                                ...
                                <td class="px-4 py-4">
                                    ...
                                    @else
                                        ...
                                        <p class="mt-1 text-[11px] text-gray-400">{{ $user->dinonaktifkan_pada?->translatedFormat('d M Y, H:i') }}</p>
                                    @endif
                                </td>
                                <td class="px-4 py-4 text-right">
                                    ...
                                    @else
                                        <button type="button" wire:click="toggleNonaktif({{ $user->id }})"
                                            class="inline-flex items-center rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-medium text-emerald-700 hover:bg-emerald-100 transition">
                                            Aktifkan Kembali
                                        </button>
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr><td colspan="3" class="px-4 py-10 text-center text-sm text-gray-400">Tidak ada pengguna yang cocok.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
