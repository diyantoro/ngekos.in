...
new #[Layout('layouts.app')] class extends Component
{
    ...
    public function hapusFavorit(int $propertiId): void
    {
        Auth::user()->favorits()->detach($propertiId);
    }

    public function hapusSemua(): void
    {
        Auth::user()->favorits()->detach();
    }
}; ?>
        ...
        <div class="mt-6">
            @if ($favorits->isEmpty())
                <div class="rounded-2xl bg-white dark:bg-gray-800 shadow-sm ring-1 ring-gray-100 dark:ring-gray-700 px-6 py-16 text-center">
                    ...
                    <a href="{{ route('kos.index') }}" wire:navigate class="mt-4 inline-flex items-center gap-2 rounded-xl bg-teal-600 px-4 py-2 text-xs font-semibold text-white hover:bg-teal-500 transition">
                        Jelajahi Katalog
                    </a>
                </div>
            @else
                <div class="flex items-center justify-between mb-3">
                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ $favorits->count() }} kos tersimpan</p>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach ($favorits as $properti)
                        <div class="group bg-white dark:bg-gray-800 rounded-2xl shadow-sm ring-1 ring-gray-100 dark:ring-gray-700 overflow-hidden hover:shadow-md hover:ring-rose-200 dark:hover:ring-rose-800 transition-all duration-200">
                            ...
                            <div class="p-4">
                                <a href="{{ route('kos.detail', $properti) }}" wire:navigate>
                                    <h3 class="text-sm font-bold text-gray-900 dark:text-gray-100 group-hover:text-teal-600 dark:group-hover:text-teal-400 transition line-clamp-1">{{ $properti->nama }}</h3>
                                    <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400 line-clamp-1">{{ $properti->kota ?? $properti->alamat ?? 'Lokasi belum diisi' }}</p>
                                </a>
                                @php
