...
class ImageCropperPlugin extends ImageCropperPlatform {
  ...
  Future<CroppedFile?> cropImage({
    ...
    Cropper? cropper;
    void initCropper() {
      if (cropper == null && image.complete) {
    ...
    initializer() {
      if (image.complete) {
        ...
        image.addEventListener('load', ((web.Event _) => initCropper()).toJS);
      }
    }
    ...
    Future<String?> doCrop() async {
      while (cropper == null || !image.complete) {
    ...
    void doRotate(RotationAngle angle) {
      final c = cropper;
      if (c == null) return;
      c.rotate(rotationAngleToNumber(angle));
    }
