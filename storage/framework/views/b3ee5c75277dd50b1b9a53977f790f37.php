...
<?php endif; ?>
        ...
        <div id="growth-data"
            ...
            data-growth-pembayaran='<?php echo json_encode($chartGrowthPembayaran, 15, 512) ?>'
        ...
        <div id="analytics-data"
            data-labels='<?php echo json_encode($bulanLabels, 15, 512) ?>'
            data-user-anak='<?php echo json_encode($userGrowthMonth['anak_kos'], 15, 512) ?>'
            data-user-pemilik='<?php echo json_encode($userGrowthMonth['pemilik'], 15, 512) ?>'
            data-nilai-transaksi='<?php echo json_encode($chartNilaiTransaksi, 15, 512) ?>'
            class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm ring-1 ring-gray-100 dark:ring-gray-700 p-4 sm:p-6">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
        ...
        <div id="status-data-admin"
            data-sewaan='<?php echo json_encode($statusPenyewaan, 15, 512) ?>'
            data-pembayaran='<?php echo json_encode($statusPembayaran, 15, 512) ?>'
            class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm ring-1 ring-gray-100 dark:ring-gray-700 p-4 sm:p-6">
                <h3 class="text-base font-bold text-gray-900 dark:text-gray-100">Distribusi Status Penyewaan</h3>
                <p class="text-xs text-gray-500 dark:text-gray-400"><?php echo e($periodeBulan); ?> bulan terakhir pada properti kelolaan</p>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm ring-1 ring-gray-100 dark:ring-gray-700 p-4 sm:p-6">
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400"><?php echo e($periodeBulan); ?> bulan terakhir: menunggu verifikasi, diverifikasi, dan ditolak</p>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.dashboard-funnel','data' => ['stages' => $funnelStages,'title' => 'Grafik Pipeline','subtitle' => 'Kunjungan → Penyewa → Tagihan → Lunas, properti yang Anda kelola (keseluruhan)']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['stages' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($funnelStages),'title' => 'Grafik Pipeline','subtitle' => 'Kunjungan → Penyewa → Tagihan → Lunas, properti yang Anda kelola (keseluruhan)']); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
