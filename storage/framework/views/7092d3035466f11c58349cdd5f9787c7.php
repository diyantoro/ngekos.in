...
<div class="py-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Paket saat ini: <span class="font-bold text-gray-900 dark:text-gray-100"><?php echo e(strtoupper($paketAktif)); ?></span>. Bayar via QRIS, paket langsung aktif otomatis.</p>
        </div>
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $pakets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $paket): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                <div
                    ...
                    class="relative group flex h-full w-[84%] shrink-0 snap-center flex-col overflow-hidden rounded-2xl border p-6 transition-shadow duration-300 ease-out sm:w-[62%] md:w-auto
                        ...
                        <?php echo e($isActive ? 'ring-2 ' . $c['ring'] : ''); ?>"
                    :style="`transform: translateY(${hovered ? '-6px' : '0'}) scale(${pressed ? 0.98 : 1}); box-shadow: ${hovered ? <?php echo \Illuminate\Support\Js::from($c['hoverShadow'])->toHtml() ?> : <?php echo \Illuminate\Support\Js::from($c['restShadow'])->toHtml() ?>};`"
                >
                    
                    <div class="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r <?php echo e($c['accentGradient']); ?>"></div>

                    <div class="relative z-10 flex flex-1 flex-col pt-1">
                        <div class="flex items-center justify-between gap-2 mb-3">
                            <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wider ring-1 <?php echo e($c['badge']); ?>">
                                <?php echo e($paket['name']); ?>

                            </span>
                            <span class="inline-flex shrink-0 items-center gap-1.5">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($isPopular && ! $isActive): ?>
                        ...
                        <ul class="mt-5 flex-1 space-y-2 text-sm text-gray-600 dark:text-gray-300">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $paket['features'] ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fitur): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <li class="flex min-w-0 items-start gap-2 transition-colors duration-200 hover:text-gray-900 dark:hover:text-gray-100">
                                    <span class="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded <?php echo e($key === 'free' ? 'bg-slate-400' : ($key === 'pro' ? 'bg-teal-500' : 'bg-violet-500')); ?> text-white text-[10px] font-bold">✓</span>
